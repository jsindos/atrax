...
const mutations = {
  Mutation: {
    async createStepImage (root, args, context) {
      ...
      const { uri } = await context.services.ImageUploadService.storeImage(image, context.req.headers.host)

      const step = await context.models.Steps.findByPk(stepId)

      if (uri) {
        const currentImages = await context.models.Images.findAll({ where: { stepId }, order: [['index', 'DESC']] })
